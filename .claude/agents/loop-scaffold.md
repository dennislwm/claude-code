...
1. `loop.md`, in this order. The ORDER is load-bearing -- Bash discipline sits
   ...
   - **Boundaries** -- work only on the branch; NEVER push to any remote.
     ...
     Also never edit the loop's own governing files (`loop.md`,
     `agents/*.md`, `settings.json`, the wiki's `CLAUDE.md`): the verifier
...
The unit-of-work SHAPE is fixed, not project-specific: Dispatch (section 1
above) always generates the same "decision proposed as an ADR, else a
loop-surfaced REQ/defect fixed" structure, emitted VERBATIM per the "generic
sections" rule at the top of Generate mode. `[docs/user instructions]` is
narrower than that: it names which project documents are authoritative and
which are stale and must be ignored -- content, not shape. The first
instantiation (`13coda-cli.wiki`) restated the fixed shape in its own
project's language ("a defect fix if one is queued, else a newly discovered
pain point proposed as an ADR") -- that restatement is vocabulary, not a
structural variant -- and validated it by closing three requirements with
GATE C passing on each.
...
| Persisted subagents | For EVERY role loop.md dispatches as its own subagent call (the verifier, Discover if it does open-ended research, any future role) -- a `<wiki>/.claude/agents/*.md` exists with `description`, `model`, `effort`, `tools`, and a rubric plus an output contract; it is referenced by name in loop.md; AND every command its own rubric names exists in this file, AND every source path it tells the agent to read exists (`ls`): a stale name reads as an empty source, and one shipped naming three files its reference repo never had. `description` is not cosmetic: the `Agent` tool's `subagent_type` registry will not register a file lacking it, so an omitted `description` makes the role silently undispatchable at tick time even though the file looks complete. References run both ways: a rubric saying "run this project's own check-command per this wiki's CLAUDE.md" against a wiki that never defined that command sends the verifier after something undefined on every diff, silently. An asymmetry across roles (one dispatched role externalized, another left as inline prose + an ad-hoc `Agent()` call) is itself a finding, even with no functional bug -- it means that role's tool-use discipline lives nowhere the agent reading only its own file can see it |
...
| Loop-inbox mechanism | If `loop.md` uses `.claude/loop-inbox.md` as a discover source (per this file's own Discover section above): it is checked FIRST every tick, holds at most one pending note, and is truncated (never deleted) immediately after being read. Both content shapes are handled correctly -- no prefix means a finished candidate (skip Discover's own research entirely, proceed straight to whichever gate it needs), a `RESEARCH:` prefix means a brief (substitute for the priority-ranking step only, the full rubric -- category-tier gate, eligibility gate, evidence provenance -- still applies unchanged) |
...
| Writer / GATE D-E (if the loop records evidence) | `writer.md` fires on every closed GATE A/C cycle or STOP; each entry passes GATE D (verify) before GATE E (human accept) |
| Editor / GATE F (if the loop assembles a written output from evidence) | The editor's rubric mapping matches the project's own rubric document's current areas; the assembled output is only written after its human-approval gate, never on the evidence gate alone |
