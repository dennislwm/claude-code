...
- `~/.claude/CLAUDE.md`, `~/.claude/commands/`, and `~/.claude/skills/` are symlinks into `02claude-code`. When asked to update the global CLAUDE.md or global commands/skills, always edit via the `02claude-code/` paths directly -- never via the `~/.claude/` symlink paths.
