...
- For JSON parsing in CLI pipelines, use `jq` or tool-native `--jq` flags. Never pipe CLI output to `python3 -c` for JSON parsing -- interpreter pipelines trigger permission prompts. For base64-encoded content, pipe to `base64 -d`. This includes read-modify-write edits (update a field, append to an array), not just reads -- e.g. `jq '.last_tick = "<ts>" | .progress += [{"tick":"<ts>","result":"<r>"}]' file.json > tmp && mv tmp file.json`; there is no shape where `python3 -c` is actually necessary.
